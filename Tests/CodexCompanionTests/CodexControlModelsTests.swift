...
struct CodexControlModelsTests {
    ...
    @Test
    func buildsGoalCreationThroughTheSupportedGoalSetMethod() {
        let create = CodexControlRequestFactory.goalCreate(
            id: 4,
            threadID: "thread-4",
            objective: "Ship mobile goal controls",
            tokenBudget: 75_000
        )

        #expect(create.method == "thread/goal/set")
        #expect(create.params["threadId"] as? String == "thread-4")
        #expect(create.params["objective"] as? String == "Ship mobile goal controls")
        #expect(create.params["tokenBudget"] as? Int == 75_000)
        #expect(create.params["status"] == nil)
    }
