...
struct PetAnimationStateTests {
    ...
    @Test
    func processAndAttentionMappingsKeepThinkingExclusiveToChat() {
    ...
    @Test
    func runningProcessTrayReactionSettlesToIdleWithoutLosingFailures() {
        #expect(CompanionAppModel.processTrayAnimationState(
            for: [.running],
            hasSettledRunning: false
        ) == .running)
        #expect(CompanionAppModel.processTrayAnimationState(
            for: [.running],
            hasSettledRunning: true
        ) == .idle)
        #expect(CompanionAppModel.processTrayAnimationState(
            for: [.running, .failed],
            hasSettledRunning: true
