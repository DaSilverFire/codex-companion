...
struct CompanionPresentationTests {
    @Test
    @MainActor
    func reopeningThePetMenuAlwaysReturnsToCodexProcesses() throws {
    ...
    func autonomousRoamingPausesForTrayHiddenPetOrAttentionMessage() {
        ...
        #expect(CompanionPresentationPolicy.pausesRoaming(
            ...
            isPetVisible: true,
            hasAttentionMessage: false,
            allowsAutonomousMovement: false
        ))
    }

    @Test
    func stationaryCursorTrackingRespectsPresentationPriority() {
        #expect(CompanionPresentationPolicy.allowsDirectionalLookTracking(
            isQuickBarOpen: false,
            isPetVisible: true,
            hasAttentionMessage: false
        ))
    ...
    @Test
    func openTrayControlsRequirePetOrRevealedControlHover() {
        #expect(CompanionPresentationPolicy.showsPetMenuControls(
            isQuickBarOpen: false,
            hidesUntilHover: false,
            isPetHovered: false,
            isMenuControlHovered: false
        ))
        #expect(!CompanionPresentationPolicy.showsPetMenuControls(
            isQuickBarOpen: false,
            hidesUntilHover: true,
            isPetHovered: false,
            isMenuControlHovered: false
        ))
        #expect(CompanionPresentationPolicy.showsPetMenuControls(
    ...
    @Test
    @MainActor
    func inactiveSessionClearsStalePetAndMenuControlHover() throws {
        let suiteName = "CompanionPresentationTests-\(UUID().uuidString)"
        let defaults = try #require(UserDefaults(suiteName: suiteName))
        defer { defaults.removePersistentDomain(forName: suiteName) }
        ...
        model.setPetMenuControlHovering(true)
        #expect(model.shouldShowPetMenuButton)

        model.clearPetHoverState()

        #expect(!model.isPetPointerHovered)
        #expect(!model.isPetMenuControlHovered)
        #expect(!model.shouldShowPetMenuButton)
    }
    ...
    @Test
    func hiddenOpenTrayControlsReclaimTheirRowWithoutMovingThePet() {
        let petWindowFrame = NSRect(x: 640, y: 120, width: 124, height: 164)
        let visibleAnchor = PetWindowMetrics.trayAnchorFrame(
            for: petWindowFrame,
            areMenuControlsVisible: true
        )
        let compactAnchor = PetWindowMetrics.trayAnchorFrame(
            for: petWindowFrame,
            areMenuControlsVisible: false
        )

        #expect(visibleAnchor.minY == petWindowFrame.minY)
        #expect(
            visibleAnchor.height
                == petWindowFrame.height - PetWindowMetrics.trayBaselineDrop
        )
        #expect(
    ...
    @Test
    func petHoverPushesBothProcessAndChatTraysUpOneControlRow() {
        let visibleFrame = NSRect(x: 0, y: 0, width: 1_440, height: 900)
        let petWindowFrame = NSRect(x: 640, y: 120, width: 124, height: 164)

        for traySize in [
            CGSize(width: PetWindowMetrics.trayWidth, height: 240),
            CGSize(width: PetWindowMetrics.trayWidth, height: PetWindowMetrics.compactTrayHeight),
        ] {
            let compactOrigin = PetWindowMetrics.positionedTrayOrigin(
                anchorFrame: PetWindowMetrics.trayAnchorFrame(
                    for: petWindowFrame,
                    areMenuControlsVisible: false
                ),
                traySize: traySize,
            ...
            let revealedOrigin = PetWindowMetrics.positionedTrayOrigin(
                anchorFrame: PetWindowMetrics.trayAnchorFrame(
                    for: petWindowFrame,
                    areMenuControlsVisible: true
                ),
    ...
    @Test
    @MainActor
    func visibleTrayRefreshesAfterProcessStorePublishesWithoutReopening() async throws {
        let suiteName = "CompanionPresentationTests-process-refresh-\(UUID().uuidString)"
        let defaults = try #require(UserDefaults(suiteName: suiteName))
        defer { defaults.removePersistentDomain(forName: suiteName) }
        let store = CodexProcessStore(defaults: defaults)
        let observer = PetTrayProcessRefreshObserver()
        var refreshCount = 0
    ...
    @Test
    func directionalLookFramesRequireCursorProximity() {
        let petFrame = CGRect(x: 100, y: 100, width: 124, height: 124)
        let center = CGPoint(x: petFrame.midX, y: petFrame.midY)

        let nearby = PetDirectionalLookFrame.resolve(
            pointer: CGPoint(x: center.x + petFrame.width, y: center.y),
            petFrame: petFrame,
            startRow: 9
        )
        let distant = PetDirectionalLookFrame.resolve(
            pointer: CGPoint(x: center.x + petFrame.width * 3, y: center.y),
            petFrame: petFrame,
            startRow: 9
        )
