...
final class CompanionAppModel: ObservableObject {
    ...
    @Published var lumoAPIKeyInput = ""
    @Published var lumoAPIKeyStatus: String
    @Published var hasLumoAPIKey: Bool
    ...
    @Published var allowsAutonomousPetMovement: Bool {
        didSet {
            interactionPreferences.allowsAutonomousPetMovement = allowsAutonomousPetMovement
    ...
    @Published var selectedLumoModel: LumoModel {
        didSet {
            UserDefaults.standard.set(selectedLumoModel.rawValue, forKey: Self.lumoModelKey)
        }
    }

    private let openAIAPIKeyStore = OpenAIAPIKeyStore()
    private let openAIChatService = OpenAIChatService()
    private let lumoAPIKeyStore = LumoAPIKeyStore()
    private let lumoChatService = LumoChatService()
    ...
    private var processRunningReactionTask: Task<Void, Never>?
    private var hasSettledRunningProcessAnimation = false
    ...
    init(
        ...
        if savedChatGPTModel != restoredChatGPTModel.rawValue {
            UserDefaults.standard.set(restoredChatGPTModel.rawValue, forKey: Self.chatGPTModelKey)
        }
        let savedLumoModel = UserDefaults.standard.string(forKey: Self.lumoModelKey)
        let restoredLumoModel = LumoModel.restoringPersistedSelection(savedLumoModel)
        selectedLumoModel = restoredLumoModel
        if savedLumoModel != restoredLumoModel.rawValue {
            UserDefaults.standard.set(restoredLumoModel.rawValue, forKey: Self.lumoModelKey)
        }
        let savedChatGPTDeliveryMode = UserDefaults.standard.string(forKey: Self.chatGPTDeliveryModeKey)
        let restoredDeliveryMode = ChatGPTDeliveryMode.restoringPersistedSelection(savedChatGPTDeliveryMode)
        selectedChatGPTDeliveryMode = restoredDeliveryMode
        if savedChatGPTDeliveryMode != restoredDeliveryMode.rawValue {
            UserDefaults.standard.set(restoredDeliveryMode.rawValue, forKey: Self.chatGPTDeliveryModeKey)
        }
        ...
        let savedLumoAPIKeyExists = lumoAPIKeyStore.hasKey
        hasLumoAPIKey = savedLumoAPIKeyExists
        lumoAPIKeyStatus = savedLumoAPIKeyExists ? "Lumo API key saved for this Mac." : "No Lumo API key saved."
    ...
    var promptPlaceholder: String {
        guard let activeProcessTarget else {
            ...
            switch selectedChatGPTDeliveryMode {
            case .onDevice:
                return "Ask on device"
            case .openAIAPI:
                return "Ask ChatGPT"
            case .lumoAPI:
                return "Ask Lumo"
    ...
    var shouldShowPetMenuButton: Bool {
        CompanionPresentationPolicy.showsPetMenuControls(
            isQuickBarOpen: isQuickBarOpen,
            hidesUntilHover: hidesMenuButtonUntilHover,
            isPetHovered: isPetPointerHovered,
            isMenuControlHovered: isPetMenuControlHovered
        )
    }
    ...
    func sendPrompt(mode: RouteMode? = nil) {
        let requestedMode = mode ?? routeMode
        let selectedMode: RouteMode = requestedMode == .smart ? .chatGPT : requestedMode
        ...
        if let target {
            guard !trimmedPrompt.isEmpty else {
                ...
                status = "Type a message to \(target.action.logName.lowercased()) to \(target.title)."
    ...
    func continueCodex() {
        ...
        showCodexProcesses()
        status = activeProcessTarget == nil
            ? "Pick Reply or Steer on a Codex process."
    ...
    func setPetMenuControlHovering(_ isHovering: Bool) {
        guard isPetMenuControlHovered != isHovering else { return }
        isPetMenuControlHovered = isHovering
    }

    func clearPetHoverState() {
        isPetHovered = false
        isPetPointerHovered = false
        isPetMenuControlHovered = false
    }
    ...
    func useLumoAPI() {
        selectedChatGPTDeliveryMode = .lumoAPI
        routeMode = .chatGPT
        isQuickBarOpen = true
        isCodexProcessTrayVisible = false
        stopProcessRefreshTimer()
        isChatGPTModelPickerExpanded = false
        status = "Using Lumo API mode."
    }
    ...
    func saveLumoAPIKey() {
        do {
            try lumoAPIKeyStore.save(lumoAPIKeyInput)
            lumoAPIKeyInput = ""
            hasLumoAPIKey = lumoAPIKeyStore.hasKey
            lumoAPIKeyStatus = hasLumoAPIKey ? "Lumo API key saved for this Mac." : "No Lumo API key saved."
        } catch {
            lumoAPIKeyStatus = error.localizedDescription
        }
    }

    func clearLumoAPIKey() {
        lumoAPIKeyStore.clear()
        lumoAPIKeyInput = ""
        hasLumoAPIKey = false
        lumoAPIKeyStatus = "Lumo API key removed."
    }
    ...
    private func sendChatGPTMenuPrompt(_ trimmedPrompt: String) {
        ...
        if selectedChatGPTDeliveryMode == .lumoAPI {
            sendLumoChatPrompt(trimmedPrompt)
            return
        }
    ...
    private func sendLumoChatPrompt(_ trimmedPrompt: String) {
        guard !trimmedPrompt.isEmpty else {
            status = "Ask inside the Companion menu with \(selectedLumoModel.title)."
            chatGPTMenuResponse = nil
            return
        }

        guard let apiKey = lumoAPIKeyStore.load() else {
            status = "Add a Lumo API key in Settings to answer inside Companion."
            selectedState = .waiting
            chatGPTMenuResponse = ChatGPTMenuResponse(
                model: selectedChatGPTModel,
                sourceTitle: selectedLumoModel.title,
                prompt: trimmedPrompt,
                message: "Create a Lumo API key from Lumo's API Docs, then paste it into Codex Companion Settings. Lumo API access is included with Lumo+.",
                usageSummary: nil
            )
            return
        }

        let model = selectedLumoModel
        prompt = ""
        status = "Asking \(model.title)..."
        selectedState = .waving
        isChatGPTResponding = true
        ...
        Task {
            do {
                let result = try await lumoChatService.send(prompt: trimmedPrompt, model: model, apiKey: apiKey)
                isChatGPTResponding = false
                selectedState = .review
                status = "Answered with \(model.title)."
                chatGPTMenuResponse = ChatGPTMenuResponse(
    ...
    private func usageSummary(for result: LumoChatResult) -> String? {
        guard let inputTokens = result.inputTokens, let outputTokens = result.outputTokens else {
            return nil
        }
        return "\(inputTokens) in · \(outputTokens) out"
    ...
    private func syncAnimationWithProcesses(_ items: [CodexProcessItem]) {
        ...
        let nextState = Self.processTrayAnimationState(
            for: statuses,
            hasSettledRunning: hasSettledRunningProcessAnimation
        )
        selectedState = nextState

        if nextState == .running {
            scheduleProcessRunningReactionSettlement()
        } else if !statuses.contains(.running) {
            processRunningReactionTask?.cancel()
            processRunningReactionTask = nil
            hasSettledRunningProcessAnimation = false
        }
    }

    private func scheduleProcessRunningReactionSettlement() {
        guard processRunningReactionTask == nil,
              !hasSettledRunningProcessAnimation else { return }
        processRunningReactionTask = Task { @MainActor [weak self] in
            do {
            ...
            guard let self else { return }
            self.processRunningReactionTask = nil
            self.hasSettledRunningProcessAnimation = true
            guard self.isCodexProcessTrayVisible else { return }
            self.syncAnimationWithProcesses(self.processStore.items)
        }
    }

    private func resetProcessRunningReaction() {
        processRunningReactionTask?.cancel()
        processRunningReactionTask = nil
        hasSettledRunningProcessAnimation = false
    }

    nonisolated static func processTrayAnimationState(
        for statuses: [CodexProcessItem.Status],
        hasSettledRunning: Bool
    ) -> PetAnimationState {
        if statuses.contains(.failed) {
            return animationState(for: .failed)
        }
        if statuses.contains(.running) {
            return hasSettledRunning ? .idle : animationState(for: .running)
        }
        if statuses.contains(.completed) {
            return animationState(for: .completed)
        }
        return animationState(for: .waiting)
    }
