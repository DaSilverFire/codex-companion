...
protocol CodexAppServerRPCPerforming: Sendable {
    func perform(_ requests: [CodexRPCRequest]) throws -> [Int: CodexRPCResponse]
}

protocol CodexGoalControlling: Sendable {
...
final class CodexAppServerControlService: CodexGoalControlling, Sendable {
    static let shared = CodexAppServerControlService()

    private let client: any CodexAppServerRPCPerforming

    init(client: any CodexAppServerRPCPerforming = CodexAppServerRPCClient()) {
        self.client = client
    }
    ...
    func createGoal(
        ...
        guard !trimmedObjective.isEmpty else {
            throw CodexAppServerControlError.invalidGoalObjective
        }
        if let tokenBudget, tokenBudget <= 0 {
            throw CodexAppServerControlError.invalidGoalTokenBudget
        }
        let request = CodexControlRequestFactory.goalCreate(
            id: 2,
            threadID: threadID.trimmingCharacters(in: .whitespacesAndNewlines),
            objective: trimmedObjective,
            tokenBudget: tokenBudget
        )
        return try performGoalMutation(request)
    }
    ...
    private func performGoalMutation(_ request: CodexRPCRequest) throws -> CodexGoalSnapshot {
        let responses = try client.perform([request])
        let data = try responseData(for: request.id, in: responses)
...
struct CodexAppServerRPCClient: CodexAppServerRPCPerforming, Sendable {
...
enum CodexAppServerControlError: LocalizedError {
    ...
    case invalidGoalObjective
    case invalidGoalTokenBudget
    ...
    var errorDescription: String? {
        ...
        case .invalidGoalObjective:
            return "Enter a goal objective first."
        case .invalidGoalTokenBudget:
            return "Goal token budget must be positive."
        }
    }
}
