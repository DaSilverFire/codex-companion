...
struct OpenAIAPIKeyStore {
    static let service = "com.silverfire.codexcompanion.openai-api-key"
    static let legacyService = "com.harlin.codex-companion.openai-api-key"
    ...
    func load() -> String? {
        ...
        if let key = loadKeychainKey(service: Self.service, allowPrompt: true) {
            return key
        }

        guard let legacyKey = loadKeychainKey(service: Self.legacyService, allowPrompt: true) else {
            return nil
        }
        if let data = legacyKey.data(using: .utf8) {
            saveKeychainKey(data)
        }
        return legacyKey
    }

    private func loadKeychainKey(service: String, allowPrompt: Bool) -> String? {
    ...
    private func saveKeychainKey(_ data: Data) {
        ...
        let updateStatus = SecItemUpdate(baseQuery(service: Self.service) as CFDictionary, attributes as CFDictionary)
        if updateStatus == errSecSuccess {
            return
        }

        var item = baseQuery(service: Self.service)
    ...
    func clear() {
        SecItemDelete(baseQuery(service: Self.service) as CFDictionary)
    ...
    var hasKey: Bool {
        loadLocalKey() != nil || hasKeychainKey(service: Self.service) || hasKeychainKey(service: Self.legacyService)
    }
    ...
    private func hasKeychainKey(service: String) -> Bool {
        var query = baseQuery(service: service)
        query[kSecMatchLimit as String] = kSecMatchLimitOne
