...
private enum CodexFollowerIPCOperation: Sendable {
    case send(
        prompt: String,
        threadID: String,
        action: CodexSendAction,
    ...
    func request(requestID: String, clientID: String) -> [String: Any] {
        switch self {
...
private enum CodexFollowerIPCError: Error {
    case cancelled
    case connectionUnavailable
    case invalidFrame
...
private final class CodexFollowerIPCSession: @unchecked Sendable {
    ...
    func run() -> CodexFollowerIPCResult {
        do {
            ...
            try send(
                operation.request(requestID: actionID, clientID: clientID),
                to: fd
            )
