...
struct CodexApprovalTests {
    ...
    @Test
    func visibleApprovalPressRequiresTheRequestedTaskToBeMounted() {
        #expect(CodexVisibleApprovalRoutePolicy.canPress(
            requestThreadID: "thread-a",
            currentVisibleThreadID: "thread-a"
        ))
        #expect(!CodexVisibleApprovalRoutePolicy.canPress(
            requestThreadID: "thread-a",
            currentVisibleThreadID: "thread-b"
        ))
        #expect(!CodexVisibleApprovalRoutePolicy.canPress(
            requestThreadID: "thread-a",
            currentVisibleThreadID: nil
        ))
    }
    ...
    @Test(arguments: [
        ("Approve once", CodexApprovalDecision.approveOnce),
        ("Approve once for Port Apple runtime to Windows", .approveOnce),
        ("Approve similar commands", .approveSimilarCommands),
        ("Approve for this session", .approveSimilarCommands),
        ("Tell Codex something else", .decline),
    ])
    func matchesNativeApprovalButtonLabels(
        label: String,
        decision: CodexApprovalDecision
    ) {
        #expect(CodexVisibleApprovalButtonMatcher.matches(label, decision: decision))
    }

    @Test
    func doesNotTreatApproveOnceAsApproveSimilarCommands() {
        #expect(!CodexVisibleApprovalButtonMatcher.matches(
            "Approve once",
